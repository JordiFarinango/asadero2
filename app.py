# app.py - Backend con Flask, PostgreSQL y Eliminar Combo Personalizado
...
def init_db():
    ...
    try:
        with conn.cursor() as cur:
            ...
            # Eliminar tabla obsoleta 'inventory_presas' si aún existe
            cur.execute("DROP TABLE IF EXISTS inventory_presas;")

            # --- Insertar/Actualizar datos iniciales ---
            # (Asegúrate de que esta lógica esté presente si la necesitas, adaptada a la nueva estructura)
            # Ejemplo: Insertar usuarios si no existen
            ...
            # Insertar contadores/valores generales en inventory_info
            cur.execute("INSERT INTO inventory_info (key, value_int) VALUES (%s, %s) ON CONFLICT (key) DO NOTHING;", ('pollosEnteros', 0))
            cur.execute("INSERT INTO inventory_info (key, value_int) VALUES (%s, %s) ON CONFLICT (key) DO NOTHING;", (PRESAS_KEY, 0)) # Total de presas inicial 0
...
# --- RUTAS AGREGAR (Modificadas para Presa General) ---
...
@app.route('/api/add/presas_total', methods=['POST']) # Ruta cambiada
...
# --- RUTAS ELIMINAR (Modificadas para Presa General) ---
...
@app.route('/api/remove/presas_total', methods=['POST']) # Ruta cambiada
...
# --- RUTA ACTUALIZAR PRECIOS (Modificada para Presa General) ---
...
def update_price():
    ...
    try:
        with conn.cursor(cursor_factory=psycopg2.extras.DictCursor) as cur:
            ...
            # Verificar si se encontró el item (excepto para presa general o combo donde rowcount puede ser 0 en UPDATE)
            ...
            if not log_msg: # Si no se asignó mensaje (caso combo update fallido?)
                 conn.rollback(); return jsonify({"success": False, "message": f"Error al actualizar precio para {item_name}."}), 500
...
# --- RUTA AÑADIR COMBO PERSONALIZADO (Modificada para Presa General) ---
...
def add_custom_combo():
    ...
    # Estructura: presas_necesarias (int), productos (dict), precio (str)
    ...
    try:
        with conn_check.cursor(cursor_factory=psycopg2.extras.DictCursor) as cur_check:
            # Solo necesitamos validar productos ahora
    ...
    for item in items:
        ...
        if item_type == 'presa':
            # Ya no validamos 'name' para presa, solo sumamos cantidad
