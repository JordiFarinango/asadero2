# app.py - Backend con Flask, PostgreSQL y Total General de Presas
...
# --- Inicialización/Actualización de la Base de Datos ---
def init_db():
    ...
    try:
        with conn.cursor() as cur:
            # Tablas existentes (simplificadas)
            ...
            cur.execute("CREATE TABLE IF NOT EXISTS inventory_info ( key VARCHAR(50) PRIMARY KEY, value_int INTEGER, value_numeric NUMERIC(10,2) );") # Usar columnas separadas o JSONB
            ...
            # --- ELIMINAR TABLA inventory_presas si existe (¡BORRA DATOS!) ---
            print("Verificando si existe tabla obsoleta 'inventory_presas'...")
            ...
            # Insertar contadores generales en inventory_info
            cur.execute("INSERT INTO inventory_info (key, value_int) VALUES (%s, %s) ON CONFLICT (key) DO NOTHING;", ('pollosEnteros', 0))
            cur.execute("INSERT INTO inventory_info (key, value_int) VALUES (%s, %s) ON CONFLICT (key) DO NOTHING;", (PRESAS_KEY, 0)) # Total de presas
            cur.execute("INSERT INTO inventory_info (key, value_numeric) VALUES (%s, %s) ON CONFLICT (key) DO NOTHING;", (PRESA_PRICE_KEY, DEFAULT_PRESA_PRICE)) # Precio default presa

            # Actualizar definiciones
            # Presas por pollo ahora es un número total
            ...
            # Combos ahora usan 'presas_necesarias' (cantidad total)
            initial_combos = {
                "combo_1_8": { "presas_necesarias": 1, "productos": {}, "precio": "2.00" }, # Asumiendo 1/8 = 1 presa
                "combo_1_4": { "presas_necesarias": 2, "productos": {}, "precio": "3.50" }, # Asumiendo 1/4 = 2 presas
                "combo_1_2": { "presas_necesarias": 4, "productos": {}, "precio": "6.50" }, # Asumiendo 1/2 = 4 presas
                "combo_entero": { "presas_necesarias": initial_presas_por_pollo_total, "productos": {}, "precio": "12.00" },
                # Ejemplo con producto:
                "combo_1_4_papas": { "presas_necesarias": 2, "productos": {"Papas": 1}, "precio": "4.50" }
            }
            cur.execute("INSERT INTO definitions (key, value) VALUES (%s, %s) ON CONFLICT (key) DO UPDATE SET value = EXCLUDED.value;",
                        (COMBOS_KEY, json.dumps(initial_combos)))
            print("Definiciones actualizadas para presas generales.")
...
# --- Rutas API (Modificadas para Presa General) ---
@app.route('/')
...
def get_inventory():
    """Devuelve inventario (total presas, productos), combos y presasPorPolloTotal."""
    conn = get_db_connection();
    if not conn: return jsonify({"success": False, "message": "Error DB (I1)."}), 500
    try:
        with conn.cursor(cursor_factory=psycopg2.extras.DictCursor) as cur:
            # Obtener total presas y su precio default
            cur.execute("SELECT key, value_int, value_numeric FROM inventory_info WHERE key IN (%s, %s)", (PRESAS_KEY, PRESA_PRICE_KEY))
            ...
            presa_precio = info_data.get(PRESA_PRICE_KEY, float(DEFAULT_PRESA_PRICE)) # Usar default si no está en DB
            ...
            cur.execute("SELECT value_int FROM inventory_info WHERE key = 'pollosEnteros'")
            result = cur.fetchone(); pollos_enteros = result['value_int'] if result else 0

            # Obtener definiciones (combos y presas por pollo total)
            ...
            presas_por_pollo_total = definitions.get(PRESAS_PER_POLLO_KEY, 8) # Default a 8 si no está definido

            # Formatear combos para devolver precios como float
            combos_con_precio_float = {}
            for key, combo_data in combos_def.items():
                try:
                    combos_con_precio_float[key] = {
                        "presas_necesarias": combo_data.get("presas_necesarias", 0), # Nueva estructura
            ...
            inventory_data = {
                "inventory": {
                    ...
                    "presas_total": presas_total, # Enviar total
                    "presa_precio_default": presa_precio, # Enviar precio default
                    "productos": productos
                },
                "combos": combos_con_precio_float,
                "presasPorPolloTotal": presas_por_pollo_total # Enviar total presas por pollo
            }
...
# --- RUTAS AGREGAR (Modificadas) ---
@app.route('/api/add/pollos', methods=['POST'])
def add_pollos():
    """Agrega pollos y actualiza el total de presas."""
    ...
    try:
        with conn.cursor(cursor_factory=psycopg2.extras.DictCursor) as cur:
            # Obtener total de presas por pollo
            cur.execute("SELECT value FROM definitions WHERE key = %s", (PRESAS_PER_POLLO_KEY,))
            result = cur.fetchone(); presas_pp = result['value'] if result else 8 # Default 8
...
# --- CAMBIO: Ruta para agregar presas genéricas ---
@app.route('/api/add/presas_total', methods=['POST'])
def add_presas_total():
    """Agrega una cantidad al total general de presas."""
    req_data = request.get_json(); cantidad = req_data.get('quantity')
    if not isinstance(cantidad, int) or cantidad <= 0: return jsonify({"success": False, "message": "Cantidad inválida."}), 400
    conn = get_db_connection();
...
# --- RUTA VENDER (Modificada para Presa General) ---
...
def sell_cart():
    ...
    try:
        with conn.cursor(cursor_factory=psycopg2.extras.DictCursor) as cur:
            # Cargar definiciones y stock actual
            cur.execute("SELECT key, value FROM definitions")
            definitions = {row['key']: row['value'] for row in cur.fetchall()}
            ...
            if requerimientos['presas_total'] > 0:
                cur.execute("UPDATE inventory_info SET value_int = value_int - %s WHERE key = %s", (requerimientos['presas_total'], PRESAS_KEY))
...
# --- RUTAS ELIMINAR (Modificadas) ---
@app.route('/api/remove/pollos', methods=['POST'])
def remove_pollos():
    """Elimina pollos y descuenta del total de presas."""
    req_data = request.get_json(); cantidad = req_data.get('quantity')
    if not isinstance(cantidad, int) or cantidad <= 0: return jsonify({"success": False, "message": "Cantidad inválida."}), 400
    conn = get_db_connection();
...
# --- CAMBIO: Ruta para eliminar presas genéricas ---
@app.route('/api/remove/presas_total', methods=['POST'])
def remove_presas_total():
    """Elimina una cantidad del total general de presas (merma)."""
    req_data = request.get_json(); cantidad = req_data.get('quantity')
    if not isinstance(cantidad, int) or cantidad <= 0: return jsonify({"success": False, "message": "Cantidad inválida."}), 400
    conn = get_db_connection();
...
def update_price():
    # --- CAMBIO: Modificado para precio de presa general ---
    ...
    if not item_type or new_price_str is None: return jsonify({"success": False, "message": "Faltan datos (tipo, precio)."}), 400
    # item_name no es necesario si editamos el precio general de presas
    if item_type not in ['producto', 'combo', 'presa']: return jsonify({"success": False, "message": "Tipo de item inválido."}), 400
    if item_type != 'presa' and not item_name: return jsonify({"success": False, "message": "Falta nombre para producto/combo."}), 400
    ...
    try:
        with conn.cursor(cursor_factory=psycopg2.extras.DictCursor) as cur:
            ...
            if item_type == 'producto':
                ...
                rowcount = cur.rowcount # INSERT ON CONFLICT devuelve 0 o 1
            ...
            if rowcount < 0 or (rowcount == 0 and item_type == 'producto'): # Si update falló o no encontró producto
                 conn.rollback(); return jsonify({"success": False, "message": f"Item '{item_name}' no encontrado o error."}), 404
...
def add_custom_combo():
    # --- CAMBIO: Adaptado para 'presas_necesarias' ---
    ...
    # Construir estructura: presas_necesarias (int), productos (dict), precio (str)
    new_combo_data = {"presas_necesarias": 0, "productos": {}, "precio": str(combo_price)}
    valid_item_types = ['presa', 'producto']
    ...
    try:
        with conn_check.cursor(cursor_factory=psycopg2.extras.DictCursor) as cur_check:
            # No necesitamos presas válidas aquí, solo productos
    ...
    for item in items:
        ...
        if item_type == 'presa':
            new_combo_data['presas_necesarias'] += item_qty # Sumar al total de presas genéricas
            ...
            new_combo_data['productos'][item_name_cap] = new_combo_data['productos'].get(item_name_cap, 0) + item_qty # Sumar si se añade el mismo producto varias veces
